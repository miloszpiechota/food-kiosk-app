...
export function BasketReviewPage({
  ...
  async function handleCreateOrderSnapshot() {
    ...
    setInfoMessage(null);
    try {
      const order = await onCreateOrderSnapshot();
      setCreatedOrder(order);
    } catch (reason) {
      setError(
        reason instanceof Error
          ? reason.message
          : "The order snapshot could not be created.",
      );
    } finally {
      setPendingAction(null);
    }
      ...
      <KioskShell className="grid place-items-center p-6">
        <section className="w-full max-w-xl rounded-3xl border border-border bg-card p-8 text-center shadow-2xl shadow-black/20">
          ...
          <p className="mt-2 text-base text-muted-foreground">
            Payment can be connected after this backend-confirmed order state.
          </p>
          ...
          <button
            ...
            className={`mt-8 min-h-14 rounded-2xl border border-primary/60 bg-primary px-8 text-base font-black text-primary-foreground shadow-xl shadow-primary/20 transition hover:bg-primary/90 active:scale-95 ${focusRing}`}
          >
            Start new order
          </button>
        </section>
          ...
          <div className="flex h-full flex-col">
            <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
              ...
              <section className="rounded-3xl border border-border bg-background/50 p-5">
                ...
                <p className="mt-2 text-base leading-6 text-foreground">
                  The next backend step is creating an immutable order snapshot
                  from this basket, then starting payment.
                </p>
              </section>
            </div>
            ...
            <div className="border-t border-border p-5">
              <button
                ...
                onClick={() => void handleCreateOrderSnapshot()}
              ...
              <p className="mt-3 text-center text-sm text-muted-foreground">
                Payment flow comes after this backend order state.
              </p>
            </div>
          </div>
        </aside>
...
function OrderModeSelector({
  ...
  const modes: Array<{ icon: typeof Utensils; label: string; value: OrderMode }> = [
    ...
    <section className="rounded-3xl border border-border bg-background/50 p-5">
      ...
      <div className="mt-4 space-y-2 border-t border-border pt-4">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-semibold text-muted-foreground">Not applied</span>
        </div>
      </div>

      <div className="mt-4 flex items-center justify-between border-t border-border pt-4">
        <span className="text-lg font-black text-foreground">Total</span>
        <Price cents={totalCents} className="text-3xl font-black text-primary" />
      </div>
    </section>
  );
}
